...
status: 確定（実装セッションはこの文書を起点に作業する）
purpose: この保管庫だけを見て、Opus（実装担当Claude）がサイト立ち上げを完遂できるようにする仕様書。
related:
  - "[[00_設定/英語圏版_上流設計]]"
...
> **あなた（実装セッションのClaude）へ**：これは英語圏向けサイト The Strolling Husband の立ち上げ仕様書。設計はすべて確定済み。**設計の再検討はしない**（疑問があれば実装を止めてユーザーに確認）。作業のたびに `logs/activity.md` に記録すること。

## 0. 必読順（実装前に読む）

1. [[CLAUDE.md]]（保管庫ルール）→ [[wiki/index]]
...
## 2. フェーズ0：ユーザー作業（Opusは手順書を提示して誘導する）
...
- `raw/` を編集しない。日本語版の棚上げドキュメント（MVV・ペルソナ・keyword-map等）を書き換えない。
- 2026/12前に報酬が発生しうる状態を作らない（§8）。
- 設計の独自変更（配色・柱・カテゴリ・収益方針）。疑問はユーザーへ。
- 記事の一人称・体験部分の捏造（voice-guide-en §執筆体制に従う）。
